...
<aside id="help-panel" role="dialog" aria-modal="false" aria-labelledby="help-title" aria-hidden="true">
  <header><span class="help-mark">__BRAND__</span><h2 id="help-title">Assistant</h2><div class="help-actions"><button id="help-expand" type="button">Expand</button><button id="help-clear" type="button">Clear</button><button id="help-close" type="button" aria-label="Close assistant">Close</button></div></header>
...
<style>
#help-launch{position:fixed;right:18px;bottom:18px;z-index:130;border:1px solid var(--line);background:var(--bg-raise);color:var(--amber);padding:10px 15px;font:600 12px "IBM Plex Mono",monospace;cursor:pointer;box-shadow:0 6px 24px rgba(30,26,23,.14)}
#help-panel{position:fixed;z-index:140;inset:0 0 0 auto;width:clamp(360px,32vw,480px);background:var(--bg);border-left:1px solid var(--line);box-shadow:-12px 0 36px rgba(30,26,23,.18);display:flex;flex-direction:column;transform:translateX(102%);transition:transform .18s ease;color:var(--text);font:14px/1.55 "IBM Plex Mono",monospace}
...
#help-panel header{height:72px;display:flex;align-items:center;gap:12px;padding:0 28px;border-bottom:1px solid var(--line);background:var(--bg-raise)}
.help-mark{font:22px "VT323",monospace;color:var(--amber)}#help-panel h2{font:400 29px "VT323",monospace;letter-spacing:1px}.help-actions{margin-left:auto;display:flex;gap:4px}.help-actions button{border:0;background:transparent;color:var(--text-dim);font:11px "IBM Plex Mono",monospace;cursor:pointer;padding:7px}.help-actions button:hover,.help-actions button:focus-visible{color:var(--amber)}
#help-thread{flex:1;overflow:auto;padding:28px;display:flex;flex-direction:column;gap:22px}.help-empty{margin:auto;max-width:320px;text-align:center;color:var(--text-dim);display:flex;flex-direction:column;gap:10px}.help-empty strong{color:var(--text);font-weight:500}.help-q{align-self:flex-end;max-width:88%;background:var(--bg-card);border-radius:19px;padding:10px 15px}.help-a{max-width:100%;white-space:pre-wrap}.help-found{color:var(--text-dim);font-size:12px;margin-bottom:12px}.help-sources{display:flex;flex-wrap:wrap;gap:8px;margin-top:15px}.help-sources a{color:var(--amber);text-underline-offset:3px}.help-tools{display:flex;gap:7px;margin-top:12px}.help-tools button,.help-error button{border:0;background:transparent;color:var(--text-dim);font:11px "IBM Plex Mono",monospace;cursor:pointer;padding:5px}.help-error,.help-noanswer{border-left:2px solid var(--amber);padding-left:12px;color:var(--text-dim)}.help-loading{color:var(--text-dim);animation:help-pulse 1s infinite alternate}@keyframes help-pulse{to{opacity:.45}}
#help-form{margin:18px 28px 28px;border:1px solid var(--amber-dim);padding:13px 15px 10px;background:var(--bg-card)}#help-input{display:block;width:100%;resize:none;border:0;outline:0;background:transparent;color:var(--text);font:14px/1.5 "IBM Plex Mono",monospace;min-height:52px}.help-compose-row{display:flex;align-items:center;color:var(--text-dim);font-size:10px;letter-spacing:1px}#help-send{margin-left:auto;border:0;background:var(--amber);color:var(--on-accent);font:11px "IBM Plex Mono",monospace;padding:9px 13px;cursor:pointer}#help-send:disabled{opacity:.4}.sr-only{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0}
...
<script>
(()=>{const $=s=>document.querySelector(s),panel=$("#help-panel"),launch=$("#help-launch"),thread=$("#help-thread"),input=$("#help-input"),store="emux.help.v1",PUBLIC_PATH="__PUBLIC_PATH__";let history=[];try{history=JSON.parse(localStorage.getItem(store)||"[]")}catch(_){history=[]}const esc=s=>{const d=document.createElement("div");d.textContent=s;return d.innerHTML};
function open(){document.body.classList.add("help-open");panel.setAttribute("aria-hidden","false");launch.setAttribute("aria-expanded","true");setTimeout(()=>input.focus(),0)}function close(){document.body.classList.remove("help-open","help-expanded");panel.classList.remove("expanded");panel.setAttribute("aria-hidden","true");launch.setAttribute("aria-expanded","false");launch.focus()}function sources(items){return '<div class="help-sources">'+items.map(x=>'<a href="'+esc(PUBLIC_PATH+(x.url||""))+'">'+esc(x.title)+'</a>').join("")+"</div>"}
