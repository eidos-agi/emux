...
#help-panel{position:fixed;z-index:140;inset:0 0 0 auto;width:min(480px,100vw);background:var(--bg,#f0ebe4);border-left:1px solid var(--line,#cabfae);box-shadow:-12px 0 36px rgba(30,26,23,.18);display:flex;flex-direction:column;transform:translateX(102%);transition:transform .18s ease;color:var(--text,#1e1a17);font:14px/1.55 "IBM Plex Mono",monospace}
#help-panel[aria-hidden="false"]{transform:translateX(0)}#help-panel.expanded{width:min(760px,100vw)}
...
@media(max-width:600px){#help-launch{right:12px;bottom:12px}#help-panel{width:100%;border-left:0}#help-panel header{height:60px;padding:0 16px}#help-thread{padding:20px 16px}#help-form{margin:10px 12px 12px}.help-actions{gap:0}#help-expand{display:none}}
...
function open(){panel.setAttribute("aria-hidden","false");launch.setAttribute("aria-expanded","true");setTimeout(()=>input.focus(),0)}function close(){panel.setAttribute("aria-hidden","true");launch.setAttribute("aria-expanded","false");launch.focus()}function sources(items){return '<div class="help-sources">'+items.map(x=>'<a href="'+esc(x.url)+'">'+esc(x.title)+'</a>').join("")+"</div>"}
